...
Camera::Camera(Vec3_t lookFrom, Vec3_t lookAt, Vec3_t up, float vfov, float aspect, float aperture, float focusDistance, float time1, float time2)
{
    m_time1 = time1;
    m_time2 = time2;
    Vec3_t u, v, w;
...
Ray Camera::getRay(Vec3_v u, Vec3_v v) const
{
    ...
    float time = m_time1 + drand48() * (m_time2 - m_time1);
    return Ray(m_origin, m_lowerLeftCorner + u * m_horizontal + v * m_vertical - m_origin, time);
}