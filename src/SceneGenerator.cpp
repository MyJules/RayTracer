    ...
    for (int a = -11; a < 11; a++)
    {
        for (int b = -11; b < 11; b++)
        {
            ...
            if ((center - Vec3_t(4, 0.2, 0)).Length() > 0.9)
            {
                ...
                {
                    res.emplace_back(std::make_unique<Sphere>(center, 0.2, std::make_shared<Lambertian>(Vec3_t(drand48() * drand48(), drand48() * drand48(), drand48() * drand48()))));
                }else if(chooseMat < 0.95) // metal
                {
                    res.emplace_back(std::make_unique<Sphere>(center, 0.2, std::make_shared<Metal>(Vec3_t(1+drand48(), 0.5*(1+drand48()), 0.5*(1+drand48())), 0.5*drand48())));
                }else // glass
                {
                    res.emplace_back(std::make_unique<Sphere>(center, 0.2, std::make_shared<Dielectric>(1.5)));
                }
            }
        }
    }
